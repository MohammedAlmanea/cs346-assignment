...
function requestingBooks(bookTag) {
  for (let i = 0; i < books.length; i++) {
    for (let j = 0; j < books[i].length; j++)
      if (
        (bookTag === books[i][j] && bookTag === bookID[i]) ||
        bookTag === bookTitle[i] ||
        bookTag === authorName[i]
      ) {
        return books[i];
      }
  }
  ...
  if (bookQuant <= book[4]) {
    if (balance >= finalPrice) {
      return (
        ...
        Thank you for purchasing from our bookstore!`) +
        (book[4] = book[4] - bookQuant)
      );
      ...
      return console.log(
        "Sorry, We didn't found the book, Please Make sure your book title is correct!"
      );
    }
  }
}
...
let balance = 350;
let wantedBook = 'Zero to One'
console.log(wantedBook);
sellingBook(wantedBook, 2, balance);
// sellingBook(wantedBook, 10, balance);
